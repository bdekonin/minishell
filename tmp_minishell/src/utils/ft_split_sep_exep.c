...
/*   Updated: 2020/09/22 13:33:40 by lverdoes      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static size_t	get_len(const char *src, char *sep, size_t start)
{
	...
	s_quot = 0;
	d_quot = 0;
	while (src[i] != '\0' && (s_quot || d_quot || !ft_charsearch(src[i], sep)))
	{
		if (!d_quot && src[i] == '\'' && (i == 0 || src[i - 1] != '\\'))
        	s_quot = 1 - s_quot;
        else if (!s_quot && src[i] == '\"' && (i == 0 || src[i - 1] != '\\'))
        	d_quot = 1 - d_quot;
		i++;
		len++;
	}
